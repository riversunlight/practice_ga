...
public:
    ...
    void init() {
        for (int i = 0; i < population_size; i++) {
            ...
            for (int j = 0; j < problem.n; j++) population[i][j] = rand() % 2;
            int constraint = 0;

            population_score[i] = problem.ObjectiveFunc(population[i], constraint);
            if (constraint) population_score[i] = 0;
        }
    }
    ...
    void recombine(int p1, int p2) {
        ...
        for (int i = 0; i < problem.n; i++) cout << population[p1][i] << " ";
        cout << endl;
        for (int i = 0; i < problem.n; i++) cout << population[p2][i] << " ";
        cout << endl;
        ...
        for (int i = 0; i < problem.n; i++) cout << offspring[0][i] << " ";
        cout << endl;
        for (int i = 0; i < problem.n; i++) cout << offspring[1][i] << " ";
        cout << endl;
    }
    ...
    void select_parents(int &p1, int &p2) {
        for (int i = 0; i < 2; i++) {
            ...
            if (histogram[population_size] == 0) sum = histogram[population_size - 1];
            cout << "sum:" << sum << endl;
            int r = rand() % sum;
            for (int j = 0; j < population_size; j++) {
                if (j == p1) continue;
    ...
    void eval_offspring() {
        ...
        for (int i = 0; i < 2; i++) {
            int constraint = 0;
            offspring_score[i] = problem.ObjectiveFunc(offspring[i], constraint);
            if (constraint) offspring_score[i] = 0;
        }
    }
    ...
    int termination_condition() {
        if (generation_cnt >= max_generation) return 1;
        return 0;
    }
